...
不需要购买服务器。新闻页面仍是静态文件，`/ops` 通过 Pages Functions、D1 和访问密码提供轻量运营后台。
...
- `ADMIN_PASSWORD` 用于 `/ops` 密码登录；不设置时默认使用代码中的备用密码。建议正式环境务必改成自己的 Secret。
...
重要：如果你之前给 `nowainews.com/admin*`、`nowainews.com/api/admin*` 配过 Cloudflare Access，请立刻停用或删除那个 Access 应用。Access 会在密码页之前拦截请求（常见表现是 302 / 空白页 / 不出现本站密码框）。新后台路径是 `/ops` 和 `/api/ops/*`，不要再给它们加 Access。
